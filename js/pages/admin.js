// ==========================================
// ЛОГІКА ПАНЕЛІ АДМІНІСТРАТОРА (ІЗОЛЬОВАНА ВЕРСІЯ)
// ==========================================
...
(function() {
    ...
    function checkAdminAccess() {
        let user = API.get('bv_current_user', null);
    ...
    function loadAllData() {
        try {
            ...
            console.error("Помилка ініціалізації даних в адмінці:", e);
        }
    }
    ...
    window.switchTab = function(tabName) {
        ...
        if(window.innerWidth < 1024) window.toggleAdminMenu();
    };
    ...
    function renderOrders(ords) {
        ...
        tbody.innerHTML = '';
        if(ords.length === 0) {
            tbody.innerHTML = '<tr><td colspan="5" class="p-4 text-center text-gray-500">Замовлень немає.</td></tr>';
            return;
        }
            ...
            tbody.innerHTML += `
                <tr class="hover:bg-white/5 transition-colors">
                    <td class="p-3 align-top" data-label="ID / Дата">
                        <div class="font-bold text-[#c5a059] text-sm">#${o.id ? o.id.toString().slice(-6) : '000000'}</div>
                    ...
                    <td class="p-3 font-semibold text-white align-top whitespace-nowrap" data-label="Сума">
                        <span class="text-base">${o.total_price || 0} ₴</span><br>
                    ...
                    <td class="p-3 align-top" data-label="Статус">
                        <select class="input-field py-2 px-2 text-xs w-full max-w-[150px]" onchange="window.updateOrderStatus('${o.id}', this.value)">
    ...
    window.updateOrderStatus = function(id, newStatus) {
        ...
        if(idx > -1) {
            ...
            window.showNotification('Статус змінено!');
        }
    };
    ...
    function renderProducts() {
        ...
        currentBatch.forEach(p => {
            const mainImg = (p.variations?.base?.images && p.variations.base.images.length > 0) ? p.variations.base.images[0] : '';
            ...
            tbody.innerHTML += `
                <tr class="hover:bg-white/5 transition-colors">
                    ...
                    <td class="p-3 font-medium text-xs w-full" data-label="Назва / Артикул">
                        <span class="text-[10px] text-[#c5a059] block mb-1">Арт: ${p.sku}</span> ${p.variations?.base?.name?.uk || 'Без назви'}
                    ...
                    <td class="p-3 text-[#c5a059] text-xs font-semibold" data-label="Ціна">${p.variations?.base?.price || 0} ₴</td>
                    <td class="p-3" data-label="Дії">
                        <div class="flex justify-end gap-4">
                            <button onclick="window.openProductModal('${p.id}')" class="text-blue-400 font-bold hover:underline uppercase tracking-wider text-[10px]">Ред</button>
                            <button onclick="window.deleteProduct('${p.id}')" class="text-red-500 font-bold hover:underline uppercase tracking-wider text-[10px]">Видал</button>
                        </div>
                    </td>
                </tr>
            `;
    ...
    function renderProductEditor() {
        ...
        let vTabs = `<button type="button" class="tab-var px-3 py-2 text-xs font-bold whitespace-nowrap ${editVar=='base'?'active':''}" onclick="window.switchVarTab('base')">Основна</button>`;
        if (actProd.sizes && actProd.sizes.length > 0) {
            actProd.sizes.forEach(s => {
                vTabs += `<div class="flex items-center tab-var px-2 py-1 text-xs font-bold whitespace-nowrap ${editVar===s?'active':''}"><button type="button" onclick="window.switchVarTab('${s}')" class="px-2">Розмір: ${s}</button><button type="button" onclick="window.removeVariationSize('${s}')" class="text-red-500 hover:text-white px-1">&times;</button></div>`;
            });
        }
        vTabs += `<button type="button" onclick="window.addVariationSize()" class="px-3 py-1 text-xs font-bold text-green-400 hover:bg-white/5 rounded ml-2">+ Додати розмір</button>`;
        
        const varTabsContainer = document.getElementById('varTabsContainer');
        if(varTabsContainer) varTabsContainer.innerHTML = vTabs;
        ...
        cont.innerHTML = `
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mt-4 border-t border-white/10 pt-4 items-end">
                <div class="md:col-span-1"><label class="text-[10px] uppercase text-gray-400 block mb-1">Тип розрахунку</label><select id="var-price-type" class="input-field bg-[#1a1a1a] font-semibold text-[#c5a059]" onchange="window.toggleVarPriceMode(this.value)"><option value="manual" ${vData.priceType === 'manual' ? 'selected' : ''}>Вручну</option><option value="auto" ${vData.priceType === 'auto' ? 'selected' : ''}>Авто (по вазі)</option></select></div>
        ...
        window.toggleVarPriceMode = function(mode) {
            ...
            if(mode === 'auto') {
                ...
                if(autoBox) { autoBox.classList.remove('hidden'); autoBox.classList.add('flex'); }
            } else {
                if(autoBox) { autoBox.classList.add('hidden'); autoBox.classList.remove('flex'); }
                if(manualBox) manualBox.classList.remove('hidden');
            }
        };

        const imgUpload = document.getElementById('varImgUpload');
        if(imgUpload) {
    ...
    window.saveActiveProduct = async function() {
        ...
        try {
            ...
            window.showNotification('Товар збережено!');
    ...
    window.deleteProduct = function(id) {
        if(confirm('Точно видалити цей товар?')) {
            ...
            filteredProducts = [...products];
            renderProducts();
            window.showNotification('Товар видалено');
        }
    };
            ...
            return children.map(c => `
                <div class="py-2 ${depth === 0 ? 'bg-white/5 p-4 rounded-xl border border-white/10 mb-3' : 'ml-4 sm:ml-6 pl-3 sm:pl-4 border-l border-white/10 mt-2'}">
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                        <div class="font-medium text-sm text-white"><span class="text-[10px] text-[#c5a059] mr-2">${c.id}</span> ${c.name?.uk || 'Без назви'}</div>
    ...
    window.deleteCategory = function(id) {
        ...
        if(confirm('Видалити категорію?')) {
            ...
            API.set('bv_categories_flat', categories);
            renderCategoriesAdmin(); window.showNotification('Видалено');
        }
    };
    ...
    window.handleImageUpload = function(event, previewId, targetInputId, mb = 5) {
        ...
        if (!file) return;
        if (file.size > mb * 1024 * 1024) { alert(`Розмір більше ${mb}MB.`); return; }
        const reader = new FileReader();
        reader.onload = e => {
    ...
    function renderBlocksAdmin() {
        ...
        list.innerHTML = homeBlocks.map(b => `
            <div class="bg-white/5 p-4 rounded-lg border border-white/10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div><div class="font-bold text-[#c5a059]">${b.name?.uk || 'Без назви'} <span class="text-[10px] text-gray-500 ml-2">(${b.id})</span></div><div class="text-[10px] ${b.active ? 'text-green-500' : 'text-red-500'} uppercase font-bold mt-1">${b.active ? 'Активний' : 'Вимкнено'}</div></div>
    ...
    window.deleteBlock = function(id) { if(confirm('Видалити цей блок назавжди?')) { homeBlocks = homeBlocks.filter(b => b.id !== id); API.set('bv_home_blocks', homeBlocks); renderBlocksAdmin(); window.showNotification('Блок видалено'); } };

    function renderBannersAdmin() {
        const list = document.getElementById('bannersListContainer'); 
        if(!list) return;
        list.innerHTML = '';
    ...
    window.deleteBanner = function(id) { if(confirm('Видалити цей банер?')) { banners = banners.filter(b => b.id !== id); API.set('bv_banners', banners); renderBannersAdmin(); window.showNotification('Видалено'); } };

    function renderExclusiveProcessAdmin() {
        const list = document.getElementById('exclusiveProcessList'); 
        if(!list) return;
        list.innerHTML = '';
    ...
    function renderExclusiveMaterialsAdmin() {
        const list = document.getElementById('exclusiveMaterialsList'); 
        if(!list) return;
        list.innerHTML = '';
    ...
    window.openMaterialModal = function(id = null) {
        ...
        if(form) form.reset(); 
        document.getElementById('material-old-id').value = ''; document.getElementById('materialModalTitle').innerText = 'Додати матеріал';
    ...
    window.saveBuilderData = function(pageId) {
        ...
        API.set('bv_pages_content', pagesContentDB); window.showNotification('Оновлено!');
    };

    window.savePriceList = function() {
        try { priceListDB = JSON.parse(document.getElementById('price-json-editor').value); API.set('bv_price_list', priceListDB); window.showNotification('Прайс-лист збережено в локальну БД!'); } catch (e) { alert('Помилка в форматі JSON!'); }
    };
    ...
    function populateSettings() {
        ['phone','tg','inst','gold-rate', 'banner-ratio'].forEach(k => {
            ...
            if(el) el.value = siteSettings[mapKey] || (k==='banner-ratio'?'3/1':'');
        });
    ...
    window.saveSiteSettings = function(event) {
        ...
        try {
            ...
            if (oldRate !== newRate && newRate > 0) {
                ...
                API.set('bv_products', products);
                renderProducts(); 
                window.showNotification('Курс оновлено, ціни перераховано!');
            } else { window.showNotification('Налаштування збережено'); }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        setTimeout(() => {
            if (typeof API === 'undefined') {
                console.error("КРИТИЧНА ПОМИЛКА: Файл api.js не завантажено!");
                alert("Помилка! Не знайдено API. Перевір шляхи до скриптів у admin.html");
                return;
            }
            checkAdminAccess();
        }, 50);
    ...
    setTimeout(() => {
        ...
        if(catForm) {
            catForm.onsubmit = (e) => {
                ...
                API.set('bv_categories_flat', categories); renderCategoriesAdmin(); populateCategorySelects(); window.closeCategoryModal(); window.showNotification('Категорію збережено');
            };
        }

        const blkForm = document.getElementById('blockForm');
        if(blkForm) {
            blkForm.onsubmit = (e) => {
                e.preventDefault();
        ...
        const bnrForm = document.getElementById('bannerForm');
        if(bnrForm) {
            bnrForm.onsubmit = (e) => {
                e.preventDefault();
